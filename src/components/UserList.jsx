    ...
    <div className="fixed inset-0 bg-gray-700 bg-opacity-80 flex items-center justify-center z-50">
      <div className="bg-white shadow-2xl rounded-lg w-full max-w-4xl relative p-6 z-50">
        ...
        <div className="flex justify-between items-center mb-4">
          <input type="text" placeholder="Search by email..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-64 p-2 border rounded-lg text-gray-700 focus:ring-2 focus:ring-blue-500" />
          <div className="flex space-x-2">
            ...
            <button onClick={() => setIsUpdateUserOpen(true)} className="flex items-center bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600">
              <PencilIcon className="w-4 h-4 mr-2" /> Change Password
            </button>
            <button onClick={() => setIsDeleteUserOpen(true)} className="flex items-center bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600">
              ...
              {filteredUsers.length > 0 ? filteredUsers.map(user => (
                <tr key={user.email} className="hover:bg-gray-100">
                  ...
                  <td className="border px-4 py-3 text-center">
                    <span className={`px-2 py-1 rounded-full ${user.is_connected ? "bg-green-200 text-green-800" : "bg-red-200 text-red-800"}`}>
